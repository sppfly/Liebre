...
public class BaseRichTuple implements RichTuple, Comparable<BaseRichTuple> {
    ...
    public boolean equals(Object o) {
        ...
        return new EqualsBuilder().append(timestamp, that.timestamp).append(stimulus, that.stimulus)
                .append(key, that.key).isEquals();
    }

    @Override
    public int hashCode() {
        return new HashCodeBuilder(17, 37).append(timestamp).append(stimulus).append(key).toHashCode();
    }

    @Override
    public String toString() {
        return new ToStringBuilder(this).append("timestamp", timestamp).append("stimulus", stimulus).append("key", key)
