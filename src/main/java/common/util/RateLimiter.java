...
/**
 * Helper class that forces a process to limits its (approximate) rate to a
 * specific number of calls per second. The thread that needs to be limited
 * needs to call the {@link #limit(long)} function at every call and the
 * {@link RateLimiter} will force it to sleep for the appropriate amount of time
 * when it starts exceeding the desired rate.
 *
 * <br />
 *
 * To prevent a spike in the calls at the beginnng of each second and a long
 * sleep afterwards, the actual implementation counts the calls in custom time
 * intervals which can be as small or as big as deemed appropriate for the
 * application.
 *
 * @author palivosd
 */
public final class RateLimiter {
     ...
     *
     * @param intervalMillis The interval where the limit measurements take place,
     *                       i.e. the time "resolution" of the limiter.
     */
    ...
    /**
     * Limit the rate of the calling thread. If more {@code rate} calls/(interval
     * millisec) have been made to this function during the limiter time interval,
     * the calling thread will sleep until the next interval.
     *
    ...
    /**
     * Update the counters depending on whether the current time belongs in the
     * current or in the next interval
     */
