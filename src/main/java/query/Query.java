...
public final class Query {
     ...
     * Construct.
     *
     * @param LiebreScheduler The LiebreScheduler implementation to use when
     *                        executing the query after Query{@link #activate()} is
     *                        called.
     */
    ...
    public synchronized <IN extends RichTuple, OUT extends RichTuple> void registerKeyByExtractor(Operator<IN, OUT> o,
    ...
    private synchronized <IN extends RichTuple, OUT extends RichTuple> Operator<IN, OUT> addTimeAggregateOperator(
            String identifier, int instance, int parallelism, long windowSize, long windowSlide,
            Window<IN, OUT> window) {

        Operator1In<IN, OUT> op = null;
        if (window instanceof TimeWindowAddSlide) {
            op = new TimeSWAggregate<IN, OUT>(identifier, instance, parallelism, windowSize, windowSlide,
            ...
            op = new TimeMWAggregate<IN, OUT>(identifier, instance, parallelism, windowSize, windowSlide,
                    (TimeWindowAdd<IN, OUT>) window);
        } else {
    ...
    public synchronized <IN extends RichTuple, OUT extends RichTuple> Operator<IN, OUT> addTimeAggregateOperator(
            String identifier, long windowSize, long windowSlide, Window<IN, OUT> window) {
        return addTimeAggregateOperator(identifier, 0, 1, windowSize, windowSlide, window);
    }

    public synchronized <IN extends RichTuple, OUT extends RichTuple> List<Operator<IN, OUT>> addTimeAggregateOperator(
            String identifier, int parallelism, long windowSize, long windowSlide, Window<IN, OUT> window) {
        ...
        if (parallelism == 1) {
            ...
            for (int i = 0; i < parallelism; i++) {
                result.add(addTimeAggregateOperator(identifier + "_" + i, i, parallelism, windowSize, windowSlide,
                        window));
            }
        }
        return result;
    ...
    private synchronized <IN, OUT> Operator<IN, OUT> addTupleAggregateOperator(String identifier, int instance,
            int parallelism, long windowSize, long windowSlide, Window<IN, OUT> window) {

        Operator1In<IN, OUT> op = null;
        if (window instanceof TupleWindow) {
            op = new TupleAggregate<IN, OUT>(identifier, instance, parallelism, windowSize, windowSlide,
    ...
    public synchronized <IN, OUT> Operator<IN, OUT> addTupleAggregateOperator(String identifier, long windowSize,
            long windowSlide, Window<IN, OUT> window) {
        return addTupleAggregateOperator(identifier, 0, 1, windowSize, windowSlide, window);
    }

    public synchronized <IN, OUT> List<Operator<IN, OUT>> addTupleAggregateOperator(String identifier, int parallelism,
            long windowSize, long windowSlide, Window<IN, OUT> window) {
        assert (parallelism >= 1);
        List<Operator<IN, OUT>> result = new LinkedList<>();
        if (parallelism == 1) {
            ...
            for (int i = 0; i < parallelism; i++) {
                result.add(addTupleAggregateOperator(identifier + "_" + i, i, parallelism, windowSize, windowSlide,
                        window));
            }
        }
        return result;
    ...
    public synchronized <IN, OUT> Operator<IN, OUT> addMapOperator(String identifier,
            MapFunction<IN, OUT> mapFunction) {
        return addOperator(new MapOperator<IN, OUT>(identifier, mapFunction));
    }

    public synchronized <IN, OUT> List<Operator<IN, OUT>> addMapOperator(String identifier,
            MapFunction<IN, OUT> mapFunction, int parallelism) {
        assert (parallelism >= 1);
        List<Operator<IN, OUT>> result = new LinkedList<>();
        if (parallelism == 1) {
    ...
    public synchronized <IN, OUT> Operator<IN, OUT> addFlatMapOperator(String identifier,
            FlatMapFunction<IN, OUT> mapFunction) {
        return addOperator(new FlatMapOperator<IN, OUT>(identifier, mapFunction));
    }

    public synchronized <IN, OUT> List<Operator<IN, OUT>> addFlatMapOperator(String identifier,
            FlatMapFunction<IN, OUT> mapFunction, int parallelism) {
        assert (parallelism >= 1);
        List<Operator<IN, OUT>> result = new LinkedList<>();
        if (parallelism == 1) {
    ...
    public synchronized <T> Operator<T, T> addFilterOperator(String identifier, FilterFunction<T> filterF) {
        return addOperator(new FilterOperator<T>(identifier, filterF));
    }

    public synchronized <T> List<Operator<T, T>> addFilterOperator(String identifier, FilterFunction<T> filterF,
            int parallelism) {
        assert (parallelism >= 1);
    ...
    public synchronized <OUT, IN, IN2> Operator2In<IN, IN2, OUT> addOperator2In(Operator2In<IN, IN2, OUT> operator) {
    ...
    public synchronized <T> Query connect(StreamProducer<T> producer, StreamConsumer<T> consumer, Backoff backoff) {
        Validate.isTrue(consumer instanceof Operator2In == false,
    ...
    public synchronized <T extends RichTuple> Query connectKeyBy(StreamProducer<T> producer,
    ...
    public synchronized <T extends Comparable<? super T>> Query connect(List<? extends StreamProducer<T>> producers,
    ...
    public synchronized <T> Query connect2inLeft(StreamProducer<T> producer, Operator2In<T, ?, ?> consumer) {
    ...
    public synchronized <T> Query connect2inRight(StreamProducer<T> producer, Operator2In<?, T, ?> consumer) {
    ...
    private synchronized <T> Stream<T> getStream(StreamProducer<T> producer, StreamConsumer<T> consumer,
            Backoff backoff) {
    ...
    public synchronized void activate() {
        ...
        LOGGER.info("Components: {} Sources, {} Operators, {} Sinks, {} Streams", sources.size(), operators.size(),
                sinks.size(), streams().size());
