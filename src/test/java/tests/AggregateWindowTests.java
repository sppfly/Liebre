...
public class AggregateWindowTests {
    ...
    // Character | to separate tests, each test 4 values: WS,WA,ts,window starts
    // (separated with /)
    ...
    public static void main(String[] args) {
        ...
        for (String test : tests.split("\\|")) {
            ...
            System.out.println("Aggregate ws " + ws + " wa " + wa + " ts " + ts + " start ts: " + winStart + " ("
                    + ta.getContributingWindows(ts) + " wins) expected: " + expectedWinStart + " ("
                    + tokens[3].split("/").length + " wins)");
